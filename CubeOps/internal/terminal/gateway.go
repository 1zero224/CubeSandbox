...
func (s *relaySession) pumpMasterToBrowser(ctx context.Context) pumpResult {
	for {
		...
		// the read limit (MaxFrameBytes+1) and validation already requires
		// StdoutPendingBytes >= MaxFrameBytes, so the old
		// `info.StdoutBytes > StdoutPendingBytes` branch could never fire.
		// A slow consumer is caught by the write deadline below.
